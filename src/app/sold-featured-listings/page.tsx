...
const SoldFeaturedListingPage = () => {
  useEffect(() => {
    const targetElement = document.getElementById(
      "sold-featured-listing-script"
    );
      ...
      script.innerHTML = `
            (function() {
              const checkIhfKestrel = () => {
                if (window.ihfKestrel) {
                  const targetElement = document.getElementById('sold-featured-listing-script');
                  if (targetElement) {
                    targetElement.replaceWith(window.ihfKestrel.render());
                  }
                } else {
  ...
  return (
    <div className="my-16 min-h-[60vh] max-w-[1440px] mx-auto px-4 sm:px-12 lg:px-16 bg-white ">
      <div id="sold-featured-listing-script"></div>
    </div>
  );
};

export default SoldFeaturedListingPage;
